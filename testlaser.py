...
# Setup the servos
horizontal_servo = board.get_pin(f'd:{horizontal_pin}:s')
vertical_servo = board.get_pin(f'd:{vertical_pin}:s')

# Load the pre-trained Haar Cascade classifier for face detection
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')

# Function to move the servos to a specific position
def move_servos(horizontal_angle, vertical_angle):
...
while True:
    ...
    if len(faces) > 0:
        ...
        move_servos(horizontal_angle, vertical_angle)
